...
public record MachineDto(int Id, string Code, string Name, string? MachineType, bool IsCnc, bool IsActive, string? SerialNumber);
    ...
    : IRequestHandler<GetMachinesQuery, Result<List<MachineDto>>>
{
    public async Task<Result<List<MachineDto>>> Handle(GetMachinesQuery req, CancellationToken ct)
    {
        ...
        var items = await query
            .OrderBy(m => m.Code)
            .Select(m => new MachineDto(m.Id, m.Code, m.Name, m.MachineType, m.IsCnc, m.IsActive, m.SerialNumber))
            .ToListAsync(ct);
...
public record MachineGroupDto(int Id, string Code, string Name, bool IsActive, int MachineCount);

public record GetMachineGroupsQuery(bool ActiveOnly = false) : IRequest<Result<List<MachineGroupDto>>>;
...
{
    public async Task<Result<List<MachineGroupDto>>> Handle(GetMachineGroupsQuery req, CancellationToken ct)
    {
        var query = db.MachineGroups.AsQueryable();
        if (req.ActiveOnly)
            query = query.Where(g => g.IsActive);

        var groups = await query.OrderBy(g => g.Code)
            .Select(g => new MachineGroupDto(g.Id, g.Code, g.Name, g.IsActive, 0))
            .ToListAsync(ct);
