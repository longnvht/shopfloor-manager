...
**`Dimension.IsFinal`**: KHÔNG liên quan tới quyền nhập của Operator — đây là cờ "QC chấp nhận tái sử dụng nguyên dung sai kỹ thuật của dimension này cho QC Final", dùng khi gộp dimsheet ở OP loại `INSP` (xem §5b, và `06_dimensions_fai.md` §3.11 cho chi tiết quy tắc gom + override `BalloonNumber`).
...
- Dimension nào QC cần dung sai khác → QC tự tạo dimension mới (cùng `BalloonNumber`) gán cho OP loại `INSP` — dimension riêng của OP INS luôn thắng nếu trùng `BalloonNumber` với dimension `IsFinal=true` từ OP trước.

**Điều kiện hiện shortcut "FAI Final" trên Dashboard** (`DashboardViewModel.canQcFinal`): OP đang xem (View Mode hoặc Operation Mode tùy theo `_work.IsViewMode`) là loại `INSP`. Chỉ role `QC Inspector`/`Administrator` thấy shortcut này.

> **Tạm thời (2026-06-24)**: đã bỏ điều kiện "sản phẩm đã hoàn thiện gia công" (`Product.StatusCode == "complete"`, §3.10 trong `06_dimensions_fai.md`) khỏi điều kiện hiện shortcut — cần quyết định lại trước khi lên production có khôi phục điều kiện này không.
>
> **Fix cùng ngày**: `canQcFinal`/`canQcInline` trước đó còn check cứng `!_work.IsViewMode` và đọc `_work.CurrentOp`/`_work.CurrentProduct` — vì QC Inspector luôn bị ép View Mode (không tạo session), 2 điều kiện này **không bao giờ true** được với QC Inspector. Đã sửa thành dùng `ctxOp`/`ctxProduct` (= `ViewOp`/`ViewProduct` khi View Mode, `CurrentOp`/`CurrentProduct` khi Operation Mode) — cùng pattern với `hasJob`/`hasOp`/`hasProd` phía trên.
>
> **Fix #2 cùng ngày**: dù shortcut đã hiện đúng, click vào vẫn không hoạt động (im lặng quay về Dashboard) — `MainViewModel.NavigateToFaiFinal()`/`NavigateToQcInline()` và `FaiViewModel.InitializeAsync()` cũng mắc lỗi tương tự (đọc `CurrentJob/CurrentOp/CurrentProduct` + check `ActiveSession.StartedAt`, đều chỉ có giá trị ở Operation Mode). Đã sửa cả 3 chỗ dùng cùng pattern View/Current-aware; bỏ check `ActiveSession.StartedAt` khỏi `NavigateToFaiFinal` (chỉ áp dụng cho Inprocess FAI của Operator, không áp dụng cho QC Inspector vốn không có `ActiveSession`).
...
### 5c. Gage Selection (✅ Done, 2026-06-23)

Ghi nhận dụng cụ đo cho mỗi kết quả — phục vụ truy vết khi gage bị thu hồi hoặc hết hạn calibration.

- `GET /api/v1/mes/gages?categoryCode=` — gage `is_valid=true`, chưa bị mượn, filter theo `Category` (nhóm rộng LIN/ANG/THD/GEO/SFC, không phải `GageType` cụ thể)
- UI: danh sách gage dạng thẻ trước input panel
- `SaveMeasureCommand` nhận `gageId` → lưu vào `MeasureValue.GageId`
- Đổi dimension → luôn chọn lại gage (không carry-over) — trừ khi cùng `BalloonNumber` đã đo ở serial khác trong Job → tự gợi ý lại (`WorkContext.LastGageIdByBalloon`, reset khi đổi Job)
- Dimension `GageTypeCode == "VIS"` (Visual Inspection) → ẩn hẳn bước chọn gage
...
| GET /api/v1/mes/gages?categoryCode= | ✅ Done (Gage Selection trong FAI, 2026-06-23) |
...
| Gage traceability | GageID bắt buộc cho mọi kết quả | Tùy chọn (không bắt buộc) — ✅ Done 2026-06-23 |
