...
export default function DocumentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/v1/tech-documents?${params}`,
  ...
  async function getDownloadUrl(docId: number) {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/v1/tech-documents/${docId}/download-url`,
      { headers: { Authorization: `Bearer ${localStorage.getItem('auth-token')}` } }
    )
    const data = await res.json()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-semibold">Tài liệu kỹ thuật{opId ? ` — OP ${opId}` : ''}</h1>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium">Code / Segment</th>
              ...
              {docs.map(doc => (
                <tr key={doc.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 font-mono text-xs">
                    {doc.code}{doc.segment ? ` [${doc.segment}]` : ''}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {doc.status !== 'Rejected' && (
                      <Button size="sm" variant="outline" onClick={() => getDownloadUrl(doc.id)}>
                        Xem
                      </Button>
                    )}
                  </td>
